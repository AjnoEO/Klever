"""Классы для форматирования в customtkinter_ext"""
...
class Formatting(Flag):
    """Флаг-класс с настройками форматирования"""
...
class FormattedString:
    ...
    def __init__(
        ...
        :param on_click: Функция, исполняемая по нажатию на ссылку 
        (удобно для создания ref-ссылок), None по умолчанию
    ...
    def __repr__(self):
        result = f'FormattedString "{self.string}"'
        if self.color:
            result += f", цвет: {self.color}"
        if self.formatting:
            result += f", форматирование: {self.formatting}"
        if self.on_click:
            result += ", с ref-ссылкой"
        return f"<{repr}>"
